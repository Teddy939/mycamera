...
## フレームレート
カメラは 30 fps の設定ですが、AF 処理側のメインループは 30 fps で動いていない可能性が高いです。
...
です。5. で 1/30 秒待機するのは、カメラのフレームバッファー更新頻度よりも細かくループしないようにしているからですが、1. 〜 4. の処理時間があるので、メインループは 30 fps で動いていないはずです (現状で計測はしていません)。
